...
getIn row col default grid =
    let
        thisRow =
            Maybe.withDefault Array.empty (Array.get row grid)
    in
...
getLiberties board point color =
    let
        thisRow =
            Maybe.withDefault Array.empty (Array.get point.row board)

        thisColor =
            Maybe.withDefault Model.Empty (Array.get point.col thisRow)

        opponentColor =
            if color == Model.Black then
                Model.White
            else
                Model.Black
    in
        if thisColor == opponentColor then
            0
        else if thisColor == Model.Empty then
            1
        else
            let
                neighbours =
                    ...
                    (\neighbour prev ->
                        let
                            ( prevBoard, prevCount ) =
                                prev
                        in
                            if (getIn neighbour.row neighbour.col Model.Empty prevBoard == thisColor) then
        ...
        PLACE_STONE row col ->
            ...
            in
                (if pointState == Model.Empty then
                    let
                        rowContents =
                            Maybe.withDefault Array.empty (Array.get row model.board)

                        tryStone =
                            { model
                                | board = Array.set row (Array.set col model.turn rowContents) model.board
                                , turn =
                                    if model.turn == Model.Black then
                                        Model.White
                                    else
                                        Model.Black
                            }
                        ...
                        neighbours =
                            getNeighbours tryStone.board point
                            ...
                            neighbours
                                |> (List.foldl
                                        (\neighbour prev ->
                                            let
                                                neighbourColor =
                                                    getIn neighbour.row neighbour.col Model.Empty prevBoard
                        ...
                        liberties =
                            getLiberties removedDeadStones point player
                    in
                        if liberties == 0 then
                            Debug.log "You have attempted suicide!" model
                        else
                            let
                                final =
                                    { tryStone
                            ...
                            in
                                if Just final.board == prevState then
                                    Debug.log "You have invoked Ko.  You may not play here this turn." model
                                else
                                    final
                 else
                    model
